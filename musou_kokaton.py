...
#         super().__init__()
#         rad = random.randint(10, 50)  # 爆弾円の半径：10以上50以下の乱数
#         color = random.choice(colors)  # 爆弾円の色：クラス変数からランダム選択
...
#     def collide_bombs(self, bombs: pg.sprite.Group):
#         """
#         爆弾との衝突判定で，範囲内の爆弾を処理（爆発エフェクトと削除）する
#         引数1 bombs：爆弾グループ
#         """
#         for bomb in bombs:
#             if self.rect.collidepoint(bomb.rect.center):
#                 # 爆発エフェクト
#                 bombs.add(Explosion(bomb, 50))
#                 # 爆弾の削除
#                 bomb.kill()
...
def main():
    ...
    while True:
        ...
        # for gras in pg.sprite.groupcollide(bombs, gras, True, True).keys():
        #     exps.add(Explosion(bomb, 50))  # 爆発エフェクト
        #     score.score_up(1)  # 1点アップ
